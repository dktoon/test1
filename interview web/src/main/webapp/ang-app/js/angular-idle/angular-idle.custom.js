...
angular.module('ngIdle.idle', ['ngIdle.keepalive', 'ngIdle.localStorage'])
  .provider('Idle', function() {
    var options = {
      ...
      interrupt: 'mousemove keydown DOMMouseScroll mousewheel mousedown  scroll',
      keepalive: true,
    ...
    this.$get = ['$interval', '$log', '$rootScope', '$document', 'Keepalive', 'IdleLocalStorage', '$window',
      function($interval, $log, $rootScope, $document, Keepalive, LocalStorage, $window) {
        ...
        //FIXME: Update code to buffer user interruption to avoid too many request
        
        $document.find('body').on(options.interrupt, function($event) {
        	clearTimeout(interruptVars.timeout);
        	if (interruptVars.first) {
        		interruptVars.first = false;
        		svc.interrupt();
        		clearTimeout(interruptVars.resetTimeout);
        		interruptVars.resetTimeout = window.setTimeout(function(){
        			interruptVars.first = true;
        		},4000);
        		return;
        	}
        	
        	interruptVars.timeout = window.setTimeout(function(){
        		 svc.interrupt();
...
//        	console.log("wrap update - interrupt");
