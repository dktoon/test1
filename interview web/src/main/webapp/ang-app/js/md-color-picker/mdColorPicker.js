	...
	.directive('mdColorPicker', [ '$timeout', 'mdColorPickerHistory', function( $timeout, colorHistory ) {
		...
		return {
			...
			controller: ['$scope', '$element', '$attrs', '$mdDialog', '$mdColorPicker', function( $scope, $element, $attrs, $mdDialog, $mdColorPicker ) {
				...
				$scope.mdColorClearButton = $scope.mdColorClearButton === undefined ? false : $scope.mdColorClearButton;
...
angular.module("mdColorPicker").run(["$templateCache", function($templateCache) {
	$templateCache.put("mdColorPicker.tpl.html","<div class=\"md-color-picker-input-container\" layout=\"row\" layout-align='start center'>" +
			...
			"	</div>" +
			"	<label style='margin-left: 8px;'><md-icon ng-if=\"icon\">{{icon}}</md-icon>{{label}}</label>" +
